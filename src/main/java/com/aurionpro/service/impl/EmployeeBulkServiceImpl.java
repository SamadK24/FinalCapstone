...
public class EmployeeBulkServiceImpl implements EmployeeBulkService {
    ...
    public List<EmployeeBulkResultDTO> processCsv(Long orgId, MultipartFile file) throws Exception {
        List<EmployeeBulkRowDTO> rows = parseCsvBasic(file);
        ...
        for (EmployeeBulkRowDTO row : rows) {
            ...
            try {
                ...
                if (!emailsSeen.add(emailLower)) throw new IllegalArgumentException("Duplicate email in file");
                if (!codesSeen.add(code)) throw new IllegalArgumentException("Duplicate employeeCode in file");
                if (userRepository.existsByEmail(emailLower)) throw new IllegalArgumentException("Email already exists");
                if (userRepository.existsByUsername(code)) throw new IllegalArgumentException("EmployeeCode already exists");
                ...
                if (row.getDateOfJoining() != null && !row.getDateOfJoining().isBlank()) {
                    LocalDate.parse(row.getDateOfJoining().trim());
                }
